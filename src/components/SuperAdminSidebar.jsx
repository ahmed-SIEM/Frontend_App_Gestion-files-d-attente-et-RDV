...
import { LayoutDashboard, CheckCircle, Building2, User, LogOut } from 'lucide-react';
import NotificationBell from './NotificationBell';

const NAV_ITEMS = [
  { to: '/superadmin/dashboard',     icon: LayoutDashboard, label: 'Dashboard' },
  { to: '/superadmin/validate',      icon: CheckCircle,     label: 'Valider Établissements' },
  { to: '/superadmin/establishments', icon: Building2,      label: 'Gérer Établissements' },
  { to: '/superadmin/profile',       icon: User,            label: 'Mon Profil' },
];
...
export default function SuperAdminSidebar() {
  ...
  return (
    <aside className="w-64 bg-gradient-to-b from-blue-900 to-blue-700 text-white flex flex-col hidden lg:flex">
      {/* Top */}
      ...
      <div className="px-6 py-4">
        <p className="text-xs text-white/50 uppercase tracking-wider mb-1">Super Admin</p>
        <p className="font-semibold text-sm truncate">{user?.nom_complet || user?.email}</p>
      </div>
